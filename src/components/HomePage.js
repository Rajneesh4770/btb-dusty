...
export default function HomePage({
  ...
  {connected ? (
    !homeLoading ? (
      ownerDusty && ethers.BigNumber.isBigNumber(ownerDusty) ? (
        new Intl.NumberFormat().format(
          parseFloat(ethers.utils.formatEther(ownerDusty)) // Ensure it's parsed to a float
        )
      ) : (
        <span>N/A</span> // If ownerDusty is not a BigNumber, display N/A
      )
    ) : (
      <Skeleton
        width={120}
        sx={{ bgcolor: '#ffffff20' }}
        height={50}
        style={{ marginLeft: "auto", backgroundColor: "ffffff3d" }}
      />
    )
  ) : (
    <span>N/A</span>
